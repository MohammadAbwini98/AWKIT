...
| `/smart-waits` | Smart Wait Engine, Runner timing, Recorder wait capture | Element appears/disappears, text changes, button enables, loader/content, delayed navigation, modal, toast, delayed API response, sequential waits, intentional failure context, and fast no-wait scenario. `wait-route-background-enable` pushes `/smart-waits/shorts/local-video` while an independent page-load timer enables `Background suggestion`; route evidence must dominate and the enable remain advisory. Exercised end-to-end by `verify:smart-wait-causality`. |
...
| `/navigation-lab` | Recorder navigation/URL tracking, navigation de-duplication | Every URL transition driven by a **real user gesture**, so the capture path is exercised the way an application produces it rather than the way a test can fake it. Controls: `nav-link` (link click → `/navigation-lab/arrived`), `nav-form` + `nav-who` + `nav-submit` (GET form submit → `/navigation-lab/arrived?who=…&via=form`), `nav-redirect` (server 302 → `/navigation-lab/arrived?via=redirect`), `nav-pushstate` / `nav-replacestate` (`history.pushState` / `replaceState` called by **this page's own script**, to `/navigation-lab/route-a?step=1` and `route-b?step=2`), `nav-hash` (`#section-2`), `nav-back` / `nav-forward` / `nav-reload` (the page's own history buttons), `nav-reset`. Status: `nav-status`, log: `nav-log`. **Expected:** exactly one record per distinct destination; back, forward and reload add none; the 302's intermediate hop is never recorded. Exercised by `verify:recorder-navigation`. |
| `/navigation-lab/arrived` | Recorder navigation destination | Destination for the link click, form submit and redirect. Echoes `via` and `who` from the query string (`arrived-via`, `arrived-who`) so a recorded URL can be checked against the route that produced it, not merely against a path. |
| `/navigation-lab/route-a`, `/navigation-lab/route-b` | SPA route reload contract | Served by the lab page itself, so a reload after `pushState` lands on the same URL instead of a 404 — without this the reload de-duplication check would be measuring a 404 rather than a repeat visit. |
...
| `/mock/popup/` | Multi-Window / Popup Flow Handling | Index of 10 popup scenarios: target blank, window.open, auto-close, stays-open, multiple popups, failure cases, smart-wait popup, reversed order, script/timer identity, and URL lifecycle. |
| `/popup/url-lifecycle.html` | Popup URL lifecycle — identity is not the creation-time URL | Scenario J, four cases where reading the popup's URL once at creation loses its identity. **J1** `open-blank-then-navigate` calls `window.open("")` and assigns `location` 120 ms later, so the page genuinely exists at `about:blank` first and must be registered as provisional, then updated to `/popup/blank-then-navigate-popup.html`. **J2** `open-redirecting-popup` opens `/popup/redirect-entry.html`, which answers a fixed `302` to `/popup/redirect-final.html` (constant destination — nothing from the request reaches `Location`, so it can never be an open redirect); the **final** URL is the identity and the intermediate hop must not be recorded as one. **J3** `open-history-popup` opens `/popup/history-popup.html`, whose `history-push`, `history-hash` and `history-back` controls change the popup's own URL without navigating — each transition belongs to the popup, never to the opener, whose URL must stay unchanged. **J4** `open-same-title-pair` opens `/popup/same-title-alpha.html` and `/popup/same-title-beta.html`, which share the exact `<title>` `Shared report title` and differ only by path, so identity must not collapse to one page. Status text: `data-testid="lifecycle-status"`; each popup carries a `*-marker` and a `*-result` status node. Exercised by `verify:popup-mock-site` (14–17). |
| `/popup/multiple.html` | Three-page switching (opener + two popups) | Scenario E. `open-first-popup-button` and `open-second-popup-button` open `/popup/multiple-popup-{1,2}.html`. **Note both popups' action buttons call `window.close()`**, so clicking a popup's own control closes it — any test asserting page isolation must check foreign controls BEFORE clicking own controls. `opener-marker-button` exists precisely because the two opener buttons have side effects: it only writes `multiple-status`, so a step can prove it landed on the opener without creating a third page (a second instance of the same URL makes the popup alias legitimately ambiguous). Exercised by `verify:popup-identity` Suite 8b and `verify:popup-mock-site`. |
...
| `/dialog-lab` | Native JavaScript dialogs (`FlowStep.dialogExpectation`) | Three controls open exactly one `alert`, `confirm` or `prompt` each and write the outcome the page ITSELF observed into `data-testid="dialog-result"` — so a test can never confuse "the click happened" with "the dialog was answered". `confirm` reports `confirm accepted` / `confirm dismissed`; `prompt` distinguishes an accepted-but-empty answer (`prompt answered: `) from a dismissal (`prompt dismissed`), because `""` and `null` are different answers. `open-chained` opens a `confirm` and then a `prompt`, so a step declaring only the first proves the second still gets the documented default. `open-nothing` opens no dialog at all — the negative case for a declared expectation. Exercised by `verify:dialogs`. |
| `/storage-lab` | Browser-storage assertions (`assertionType: "storage"`) | Controls that write, empty, remove and delay-write `localStorage` / `sessionStorage` keys while a visible banner reports what the page BELIEVES it did. `storage-fake-signin` updates only the banner and never touches the store — the case where a text assertion passes and a storage assertion must not. `awkit-empty` separates a key holding `""` from one that was removed; `awkit-scope` is written to `sessionStorage` only, so reading it from `localStorage` must still report absent. `storage-write-delayed` writes `awkit-delayed` 600 ms after an immediately-optimistic banner and only then sets `storage-delayed-confirmed`, giving a flow an observable signal to synchronise on instead of a clock. Exercised by `verify:storage-assertions`. |
| `/popup/document-write.html` | Recorder capture in a `document.write` popup | Opens a blank window and builds its whole document with `document.write` — how generated report, print and confirmation tabs are made. The implicit `document.open()` removes every listener on the Window, Document and its nodes while leaving Window properties untouched, so a window-scoped install flag still reads "installed" for a page listening to nothing. The popup never commits a URL either, so popup identity resolution always spends its full budget on it. Stable markers: `open-written-popup`, `written-confirm`, `written-confirmed`. Exercised by `verify:recorder-capture-gaps` [E]. |
| `/capture-gaps` | Recorder capture shapes WebDriverUniversity exposed | Five fixtures that each produced a recording which looked fine and could not replay. `cg-source` is a drag whose element follows the cursor, so the drag ghost — not `cg-target` — is the topmost hit at the release point (`cg-holder` reserves its space, or the drop zone would slide out from under the cursor and the fixture would stop testing occlusion). `cg-colours` is a radio group with no id or label, identified only by `value`. `cg-list-host` re-renders on a timer with items whose own text is their only identity, while the wrapper carries their aggregate text — which is a summary, not an identity. `cg-readonly-input` opens a picker and can never fire `change`, so its click IS the interaction. `cg-destroy-inner` is replaced by its parent on `mousedown`, and the browser fires no `click` at all when the mousedown target is removed. Exercised by `verify:recorder-capture-gaps`. |
...
| `/scroll-lab` | Recorder scroll capture (AWKIT-REC-042), runner scroll node | Long page with below-the-fold lazy block (`data-testid="lazy-item"`, clicked state `lazy-clicked`). Wheel gestures must be captured as PAGE-level scroll nodes so replay mounts and reaches the lazy content. Exercised by the third-pass recorder verifier. |
...
| `/iframe-nested` | `LocatorContext.frameChain` (C1) | Two-level same-origin nesting (main → `#frame-outer` → `#frame-inner` → `/iframe-nested-leaf`). The leaf's `data-testid="nested-apply"` button posts to `window.top`, mirrored at `nested-mirror`, so a top-level assertion confirms the deep click. Only the ordered outer→inner frame chain reaches it; a dropped or reordered chain cannot. The **cross-origin**, duplicate-iframe, navigate-after-attach and identity-change frame-chain cases are covered by the self-contained `npm run verify:frame-chain` (two mutually cross-origin `127.0.0.1` ports). |
| `/shadow-frame-child` | Recorder Shadow DOM frame boundary | Deterministic child document used twice by `/recorder-lab`: same-origin through `localhost`, and cross-origin through `127.0.0.1`. It contains one open-shadow role button plus an observable result. The **real recorder now resolves cross-origin frames** via `LocatorContext.frameChain` (C1, gated by `verify:frame-chain`); the `verify:recorder` harness installs a plain `__awtkit_recordAction` binding that skips frame-chain building, so through THAT harness the cross-origin fixture still exercises the review-required path. |
| `/closed-shadow-lab` | Instrumented closed-shadow (C2) | A web component (`data-testid="secret-widget"`) mounts an `Apply setting` button inside a **closed** shadow root; the button posts `applied` to `window.top`, mirrored at `cs-result`. The recorder captures an instrumented host-chain locator and the runner replays it via the closed-shadow bridge (built-in engines cannot pierce a closed root). The internal accessible name is deliberately not persisted. Nested / mixed-open-closed / missing-bridge / changed-host / token-privacy cases are the self-contained `npm run verify:closed-shadow`. |
| `/drag-lab` (press-and-hold section) | `clickAndHold` gesture capture and replay | `hold-box` renders one state while the button is DOWN and another once it is released, and records how long the page itself measured it held (`hold-observed-ms`) so replay fidelity is asserted rather than assumed. `inert-box` reacts to nothing, so a long press on it must stay an ordinary click — recognising a hold on evidence-free duration alone would be a guess. Exercised by `verify:click-and-hold`. |
| `/drag-lab` | Recorder drag capture, `drag` step type, StepExecutor drag replay | A kanban board (`data-testid="drag-drop-section"`) with three native HTML5 `draggable` cards (`drag-card-design`/`drag-card-build`/`drag-card-ship`) starting in the **To Do** column and two empty drop columns (`drag-col-todo`/`drag-col-doing`/`drag-col-done`). Dropping a card into a column moves the node and reports `"<card> → <column>"` in `data-testid="drag-result"` (also on `window.__dragLab`). Listeners are delegated on the stable `.drag-board`, so `data-testid="drag-reset"` (which swaps innerHTML) never leaves the new cards without handlers. The Recorder captures a drag as one `drag` step (source + drop-target locators); the runner replays it with `source.dragTo(target)`. A second **pointer-driven sortable** (`data-testid="pointer-sort-section"`) reorders three items (`pointer-item-a`/`-b`/`-c`) with `pointerdown → pointermove → pointerup` only (no native `draggable`, like react-dnd/dnd-kit/SortableJS), reporting the order in `data-testid="pointer-order"` and the move in `pointer-result`; `pointer-reset` restores it. The Recorder's bounded pointer-gesture recognizer captures this path too. Exercised by `npm run verify:mock-site` (drives real `dragAndDrop` and a raw `page.mouse` pointer drag) and `npm run verify:recorder-competitive`. |
...
| `/blueprint-recovery-lab` | Recorder blueprint capture, LocatorFactory second-layer recovery | A same-name decoy appears early, while the real target sits beyond 205 deterministic fillers and is uniquely recordable through `data-test="blueprint-primary"`. Mutation removes every saved locator, inserts one node before the target, and changes one of three ancestry segments so the browser-captured fingerprint scores **0.866667** (above the 0.86 recovery threshold). A second mutation changes one identity token and must be refused below threshold. Stable controls: `blueprint-mutate`, `blueprint-mutate-below-threshold`, `blueprint-reset`; status: `blueprint-mutation-status`, `blueprint-result`. Exercised by `verify:mock-site` and `verify:blueprint-recovery-browser`. |
| `/recorder-lab` → Nested custom-element action ownership | Recorder action-owner normalization | Component-style nested SVGs sit inside named links and buttons. Repeated `id="icon"` values are intentionally invalid, reproducing third-party component internals that must never become the captured target. Stable selectors: `custom-owner-subscriptions-icon`, `custom-owner-settings-icon`, `custom-owner-command-inner`, `custom-owner-bare-inner`, `custom-owner-ambiguous-icon`; status: `custom-owner-result`. The duplicate buttons deliberately have no stable distinguishing container or context, proving they remain review-required. The fixture proves semantic owners win, true custom controls remain owners, and bare custom controls are conservative fallbacks. Exercised by `verify:recorder-action-owner`. |

| `/recorder-lab` keyboard shortcut fixture | Recorder keyboard shortcut capture/replay | `data-testid="keyboard-shortcut-capture"` covers canonical trusted-key capture for Ctrl/Alt/Meta combinations, Enter/Escape/Tab/Shift+Tab, and F1-F12. `shortcut-result` is the replay oracle and `shortcut-text` proves ordinary typing remains one compacted Fill rather than per-key actions. Exercised by `verify:recorder-hotkeys`. |
