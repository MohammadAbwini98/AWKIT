 ...
 *   the Chromium fallback argument, error classification, and log-payload safety.
...
function unitChecks(): void {
  ...
  console.log("\n[4] Chromium fallback launch argument");
  const base = ["--no-first-run", "--disable-sync"];
  check(
    "not added when the bypass is off",
    !applyChromiumCertificateFallbackArgs(base, { ignoreHttpsErrors: false, requiresChromiumCertificateFallback: true }).includes(
      CHROMIUM_CERTIFICATE_FALLBACK_ARG
    )
  );
  check(
    "not added when the launch path does not require it (the default for every AWKIT path)",
    !applyChromiumCertificateFallbackArgs(base, { ignoreHttpsErrors: true, requiresChromiumCertificateFallback: false }).includes(
      CHROMIUM_CERTIFICATE_FALLBACK_ARG
    )
  );
  const withFallback = applyChromiumCertificateFallbackArgs(base, {
    ignoreHttpsErrors: true,
    requiresChromiumCertificateFallback: true
  });
  check("added when required", withFallback.includes(CHROMIUM_CERTIFICATE_FALLBACK_ARG));
  check("existing args preserved and input not mutated", withFallback.length === base.length + 1 && base.length === 2);
  const twice = applyChromiumCertificateFallbackArgs(withFallback, {
    ignoreHttpsErrors: true,
    requiresChromiumCertificateFallback: true
  });
  check(
    "never duplicated",
    twice.filter((arg) => arg === CHROMIUM_CERTIFICATE_FALLBACK_ARG).length === 1,
    `count=${twice.filter((a) => a === CHROMIUM_CERTIFICATE_FALLBACK_ARG).length}`
  );
  check(
    "never added for non-Chromium browsers",
    !applyChromiumCertificateFallbackArgs(base, {
      ignoreHttpsErrors: true,
      requiresChromiumCertificateFallback: true,
      browser: "firefox"
    }).includes(CHROMIUM_CERTIFICATE_FALLBACK_ARG)
  );
  check("env escape hatch is OFF by default", isChromiumCertificateFallbackEnabled({}) === false);
  check("env escape hatch requires the exact string 'true'", isChromiumCertificateFallbackEnabled({ AWKIT_CERT_FALLBACK_LAUNCH_ARG: "1" }) === false);

  console.log("\n[5] Shared-pool compatibility key");
  ...
  check(
    "context-level bypass does NOT partition the pool (key unchanged)",
    sharedCompatibilityKey(cfg) === sharedCompatibilityKey(cfg, undefined, { certificateFallbackArg: false })
  );
  check(
    "browser-level fallback DOES partition the pool",
    sharedCompatibilityKey(cfg, undefined, { certificateFallbackArg: true }) !== sharedCompatibilityKey(cfg)
  );
