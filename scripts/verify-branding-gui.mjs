...
await navClick(win, "Settings");
await win.getByRole("heading", { name: "Appearance — Accent Color" }).first().waitFor({ timeout: 8000 }).catch(() => {});
check("Administrator can open Settings (Accent card present)", (await win.getByRole("heading", { name: "Appearance — Accent Color" }).count()) >= 1);
