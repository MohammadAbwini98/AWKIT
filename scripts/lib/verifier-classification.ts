...
export const VERIFIER_CLASSIFICATION: Record<string, VerifierClassification> = {
  ...
  "verify:locator-guard": { class: "real-browser", why: "awkit-65g Phase B guarded-positional gate — records a sensitive positional click in real Chromium, then drives buildRecordedFlow (guard hashing + sensitivity policy), FlowValidator preflight, and LocatorFactory.resolveGuardedPositional/StepExecutor to prove the runtime re-proves the recorded target identity and aborts with SENSITIVE_TARGET_IDENTITY_CHANGED on candidate-set or identity change (never a sibling fallback), with the unchanged-replay case doubling as fingerprint parity." },
