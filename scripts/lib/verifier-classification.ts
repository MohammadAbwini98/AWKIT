...
export const VERIFIER_CLASSIFICATION: Record<string, VerifierClassification> = {
  ...
  "verify:ai-error-quality-live": {
    class: "real-browser",
    why: "Gate on the owner-installed runtime and the downloaded Qwen3.5-0.8B pack (NOT RUN without them; a pack that is not the published object is refused): launches a real Electron app directory that sends L5's labelled set, built through L5a's real EvidenceBuffer and deriveFailureCause with every event labelled cause or unrelated, through analyzeFailure, the production AiService with FAILURE_ANALYSIS_LIMITS and the real ai-host.cjs. Hard: each batch coalesces as labelled (500 rows cost one call, a 409 and a 422 two), a pass and an insufficient baseline cost none, every other row is the product's request with its own deadline, delivered and saved, no canary or residual secret, every citation shown whole. Recorded: L5's metrics (baseline and AI accuracy, improvement over the baseline, false attribution, evidence-link accuracy, coalescing, calls per batch, latency) and whether the AI beats the baseline, ROADMAP rule 7's condition for automatic analysis. A scripted control of the judge runs first and ends the run if it fails."
  },
