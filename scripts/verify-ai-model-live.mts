 ...
 *
 * Run: npm run verify:ai-model-live
 */
...
if (path.basename(candidate) === MODEL_FILE_NAME) {
  check(
    "the downloaded pack matches the published object",
    measured.sha256 === PUBLISHED_PACK.sha256 && measured.sizeBytes === PUBLISHED_PACK.sizeBytes,
    `published ${PUBLISHED_PACK.sha256}/${PUBLISHED_PACK.sizeBytes}`
  );
}
