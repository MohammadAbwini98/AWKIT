...
`verify:zvec-coexistence`, `validate:offline`. The contract has only been driven against the
**staged** host tree, and `resources/dependency-manifest.json` still records
`hostProtocolVersion: 1` (it is regenerated by packaging — do not hand-edit).

**Next:** bd **`awkit-9yv`** for the packaged/NSIS layouts, and wiring the orchestrator to a real
generation root (it is currently verified against in-memory stores and a lifecycle stub only).
