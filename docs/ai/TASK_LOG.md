...
- **Objective:** clear the two remaining `verify:packaged-walkthrough` failures, both of which were
  the un-built NSIS installer.
- **`npm run package:installer` succeeded from a clean tree.** The earlier failure was the strict
  preflight — "Strict mode requires the dependency manifest to come from a clean source tree" — doing
  its job, not a defect. Provenance: `commit: 027b778, treeDirty: false`.
- **Result: 88 passed / 0 failed** (was 85/2). Part L now verifies the installer is bit-exact: the
  sha512 in a freshly regenerated `dist/latest.yml` matches the computed hash of the new
  `SpecterStudio Setup 0.1.15.exe`, size included. The `latest.yml` previously on disk was a day old
  and would have compared the new installer against a stale digest.
- **Part M observed the offline boundary:** no non-loopback TCP connections from any app process
  across the whole walkthrough (96 loopback, all mock-site/DevTools), and no zombie processes at
  teardown.
- **Cost:** the walkthrough minted one more short-lived `trial` licence with production `key2`;
  `issuance-history.jsonl` 22 → 23 lines, deliberately not pruned.
- **Housekeeping:** the release-owned `resources/dependency-manifest.*` regenerated by packaging was
  restored, not committed.
- **NOT claimed:** the clean/offline Windows VM walkthrough remains a separate human gate.
- **Files:** `docs/ai/*` only — no product or verifier source changed.


## 2026-08-20 - Claude - Run verify:packaged-licensing against a key2 build

- **Objective:** close the standing external gate — run packaged licensing against a build carrying
  `key2`.
- **Rebuilt first, on evidence.** The `dist/win-unpacked` tree was the v0.1.15 vintage and the gate
  has **no freshness guard**. Fingerprinting its `app.asar` (not its mtime) showed it carried `key2`
  but still had the issuer-page leak — 8 markers — and no `dialogExpectation`. A fresh portable from
  `ce72080` reports **0** leak markers.
- **Result: 40 PASS / 0 FAIL / 0 BLOCKED** — the first run of this gate with nothing blocked.
- **Found and fixed a regression I had introduced.** The `awkit-vf9r` CLI fold broke
  `mintVerificationLicense` three ways (`--out` removed; `--type packaged-verification` not in the
  issuer allowlist; `--expires` without `--valid-from`). It was invisible because both minting cases
  report BLOCKED when the issuer key env var is unset — **a break hidden behind a gate that does not
  run by default**. Proven by replaying the helper's exact argv against a nonexistent key, which
  failed before anything could be signed. Fixed in the helper; the service was not loosened.
- **Walkthrough assertion updated:** it asserted `licenseType === "packaged-verification"` on a
  MINTED licence, which the production issuer now refuses. It asserts the issuable type plus a short
  window instead — a stronger claim, since a production licence is not a trial expiring the same day.
- **Cost recorded:** three short-lived `trial` licences were minted with the production `key2`;
  `issuance-history.jsonl` went 17 → 20 lines and was deliberately not pruned.
- **Housekeeping:** the packaging run overwrote `dist/SpecterStudio 0.1.15.exe`. The as-released
  binary was preserved beforehand and restored afterwards (md5 `2df47c8b…` verified both ways); the
  new one is kept as `SpecterStudio 0.1.15 (ce72080 verification build).exe`. The release-owned
  `resources/dependency-manifest.*` regenerated by packaging was restored, not committed.
- **Files:** `scripts/helpers/packaged-license.mts`, `scripts/verify-packaged-walkthrough.mts`,
  `docs/ai/*`.
- **Result:** packaged-licensing 40/0/0; packaged-walkthrough 85/2 (both = un-built NSIS installer);
