...
- The L3 §10 panel fetched `listUpgrades(flowId)` — a per-FLOW view — in a loader keyed on `flowId`, and
  separately cleared the cached view whenever the selected STEP changed. Nothing refetched on a step change,
  so from the second step onwards the view stayed `null` and every step reported "no AI suggestion" even
  though main was returning its proposal. The per-step lookup already isolated the steps; clearing the view
  was both unnecessary and destructive.
- **The shape to watch for:** a reset whose key is finer-grained than the fetch it invalidates. Scope each
  reset to the identity its own data belongs to — flow change discards the view and any in-flight response,
  step change resets only transient UI (notice, disclosure). Guarded by `verify:ai-locator-upgrade-gui`,
  which fails 55/65 when the bug is reintroduced.

## OPEN (2026-09-20) — `verify:profile-store` block 7's harness precondition is host-load sensitive

- The precondition "the concurrency gate outlasts a genuinely free writer" compares the slowest unblocked
  writer against the fastest gated one, so it is a ratio of two wall-clock measurements. Measured **1.9x
  (FAIL)** while three Chromium verifiers were running on the same host, and **17.5x (PASS)** on the same
  commit minutes later with the machine idle. Nothing in the product changed between the two runs.
- It is guarding something real — without it an unobserved overlap could be a timing artifact — so do not
  loosen it. **Re-run it on a quiet host before treating a block-7 failure as a defect**, and say which state
  the host was in when reporting the result. The 73 product checks around it passed in both runs.

## The docs NUL is RECURRENT, and writing about it is how it spreads (2026-09-20, resolved twice)

- A literal U+0000 gets into a Markdown file when an editing tool interprets an escape for that code point in
  the text being written. `Read` renders it as a space, so the file looks fine; `verify:source-hygiene` is the
  only thing that sees it.
- **It is removable with `Edit`:** the same interpretation applies to the MATCH string, so an escape for
  U+0000 there matches the real byte. That is exactly why the byte got written in the first place.
- **It came back.** The previous session fixed one occurrence and then, in three files, wrote *prose about the
  fix* that itself contained the escape — so `ef7fa81` shipped with NULs in `CURRENT_STATE.md:37`,
  `KNOWN_ISSUES.md:18` and `TASK_LOG.md:17` (this very entry was one of them), and
  `verify:source-hygiene` was 10/11 at that commit despite being reported as 11/11.
- **The rule:** when writing ABOUT this code point, name it in prose only — never inside backticks as an
  escape, and never as a "replacement text" example. Run `verify:source-hygiene` after any doc edit that
  discusses it. Now 11/11 again.

## `verify:ipc-contract` and gates that live one call away (2026-09-20, resolved)

- Kept as a pattern note. The authz scan required a permission token inside each handler body, so
  `recorder:start` — whose entire gate is `resolveRecorderBrowser`, asserting `PAGE_RECORDER` or Super User —
  was reported as a contract violation for two sessions while the production code correctly denied
  unauthorized senders.
- The scan now resolves ONE level of same-file helper indirection, and only while the helper's own balanced
  body carries a permission token, so removing the assert still fails the check (mutation-confirmed). It also
  reclassified 15 Oracle write channels that had been passing merely as TRUSTED while in fact enforcing
